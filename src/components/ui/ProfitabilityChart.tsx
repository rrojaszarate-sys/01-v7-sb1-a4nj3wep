...
import { calculateProfit, formatCurrency } from '../../utils/financial';
        ...
        {points.map((point, index) => (
          <g key={index}>
            <circle
              ...
              cy={point.y}
              r="6"
              fill={index === 1 ? '#10B981' : index === 2 ? '#EF4444' : '#3B82F6'}
              className="transition-all duration-500"
              style={{
      ...
      <div className={`bg-white rounded-lg shadow-md ${className}`}>
        <div className="p-6">
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p className="text-sm">Agregue ingresos y gastos para ver el análisis de rentabilidad</p>
          </div>
        </div>
      </div>
    );
        ...
        <div className="flex flex-col lg:flex-row items-center space-y-6 lg:space-y-0 lg:space-x-8">
            ...
            {currentChartType === 'bar' ? render3DBarChart() : 
             currentChartType === 'line' ? renderLineChart() : (
              <svg
                ...
                role="img"
                aria-label={`Gráfico de rentabilidad: ${formatCurrency(Math.abs(profitData.profit))} ${profitData.isLoss ? 'pérdida' : 'ganancia'}`}
              >
                {/* Chart segments */}
                  ...
                  return (
                    <g key={segment.label}>
                      ...
                      <path
                        ...
                        role="button"
                        tabIndex={0}
                        aria-label={`${segment.label}: ${formatCurrency(segment.value)} (${segment.percentage.toFixed(1)}%)`}
                      />
            ...
            {(currentChartType === 'donut' || currentChartType === 'pie') && (
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <div className="text-center">
                  <div className={`text-2xl font-bold ${profitData.isLoss ? 'text-red-600' : 'text-green-600'}`}>
                    {formatCurrency(Math.abs(profitData.profit))}
                  </div>
          ...
          <div className="flex-1 space-y-4 min-w-0">
            {chartSegments.map((segment) => (
              ...
              >
                ...
                <div className="text-right flex-shrink-0">
                  <div className="font-semibold text-gray-900">{formatCurrency(segment.value)}</div>
            ...
            <div className={`p-4 rounded-lg border-2 ${
              profitData.isLoss ? 'border-red-200 bg-red-50' : 'border-green-200 bg-green-50'
            }`}>
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <svg className={`w-5 h-5 ${profitData.isLoss ? 'text-red-500' : 'text-green-500'}`} fill="currentColor" viewBox="0 0 20 20">
                    {profitData.isLoss ? (
                ...
                <div className="text-right">
                  <div className={`font-bold text-xl ${profitData.isLoss ? 'text-red-600' : 'text-green-600'}`}>
                    {formatCurrency(Math.abs(profitData.profit))}
                  </div>
